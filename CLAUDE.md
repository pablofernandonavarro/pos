...
NativePHP (`nativephp/electron`) **no está instalado actualmente** (no aparece en `composer.json` ni en `vendor/`), aunque ya existen `config/native.php` y `app/Providers/NativeAppServiceProvider.php` (que usa `Native\Laravel\Facades\*`) preparados para cuando se instale. Ese provider no está registrado en `bootstrap/providers.php`, así que queda inerte hasta que alguien corra `composer require nativephp/electron && php artisan native:install electron`. No asumas que las facades de Native están disponibles en runtime a menos que eso se haya hecho.
...
# Compilar el instalador de Windows (requiere NativePHP instalado)
compilar.bat
# o
php artisan native:build windows
```

**Tests**: `php vendor/bin/phpunit` (con el PHP de Herd). SQLite en memoria, forzado en `phpunit.xml` con `force="true"` porque el `.env` de esta carpeta es el de una caja real. `tests/TestCase.php` llama a `Http::preventStrayRequests()`: ningún test habla con un Manager real, los que necesitan respuestas usan `Http::fake()`. Helpers `configurarCaja()` y `producto()`. Los mismos tests corren en `pos-native`. No hay `pint.json` (Pint usaría el ruleset por defecto).
