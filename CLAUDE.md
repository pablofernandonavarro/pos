...
- **Versiones para las cajas: GitHub Actions** (`.github/workflows/escritorio.yml`). `git tag escritorio-vX.Y.Z && git push origin escritorio-vX.Y.Z` compila en un runner Windows (mismos controles que el script: dependencias iguales, parche, tests, datos de caja prohibidos, versión en el `.env`) y publica en el Manager de producción. A mano (Run workflow) solo compila y deja el zip como artefacto. Secretos del repo: `ESCRITORIO_APP_KEY` (la APP_KEY compartida, la de `.env.escritorio`), `PUBLICAR_SSH_KEY`, `PUBLICAR_KNOWN_HOSTS`; variable `PUBLICAR_HOST`. Esa clave en el servidor solo puede correr `recibir-escritorio` (comando forzado, repo del Manager `deploy/recibir-escritorio.sh`), que verifica sha256, que el zip no traiga datos de cajas y **que la versión sea mayor que la publicada**. El script local queda para probar (`-SoloPreparar`) o emergencias.
...
- **Transacciones SQLite `IMMEDIATE`** (`App\Support\ConexionSqlite`, desde `AppServiceProvider::boot`): pantalla, cola y scheduler escriben en el mismo archivo. Con `DEFERRED`, una transacción que lee y después escribe fallaba **al instante** con `database is locked` si otro proceso escribió en el medio (busy_timeout no cubre esa carrera): la caja 2 dio 500 al abrir la caja justo con el sync de cada minuto. `transaction_mode` solo lo aplica Laravel con PHP 8.4 (escritorio); las clásicas en 8.2 quedan con `busy_timeout` 10 s. Lo reproduce `ConexionSqliteTest`.
...
- **Productos nuevos o modificados**: `pos:sync --productos` corre **cada 5 minutos** y baja solo el delta (`updated_since` = `ultima_sincronizacion_productos`). Antes el catálogo solo se bajaba al instalar o con "Sincronizar", y lo dado de alta en el Manager (a mano o por Excel) no llegaba a la caja. La marca usa el reloj del Manager (`synced_at` de la respuesta) menos 2 minutos de solape, no la hora de la caja. No toca precios por lista (`syncPrecios` trunca la tabla): un producto nuevo usa el factor de la lista hasta el próximo pull completo.
...
- **Cajeros con PIN**: son usuarios del Manager con rol `cajero` (una sucursal) o `supervisor` (una o más), con PIN y sucursales cargados en `Usuarios`. Bajan por `sync/cajeros` (en `syncInicial` y cada minuto) con el PIN en bcrypt; `cajeros.id` local = id del usuario en el Manager; se verifican localmente para funcionar offline. `AutorizacionService::verificar()` limita a 5 intentos por minuto por cajero (`RateLimiter`). **Con cajeros cargados la caja solo se abre con `CajaService::abrirConPin()`**; `abrir()` con nombre libre queda para instalaciones sin cajeros y lo bloquea el propio servicio.
